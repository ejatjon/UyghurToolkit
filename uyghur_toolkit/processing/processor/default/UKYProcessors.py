...
from uyghur_toolkit.resources.dictionaries.letter_mapping.UKY_SCRIPTS import UKY_SCRIPTS
from uyghur_toolkit.resources.dictionaries.numbers import UKY_NUMBER_MAP, UKY_REVERSE_NUMBER_MAP
from uyghur_toolkit.resources.dictionaries.symbols import PUNCTUATIONS, UKY_SYMBOL_MAP
from uyghur_toolkit.resources.dictionaries.vowels import UKY_VOWELS
...
def int_to_uky(num: int) -> str:
    ...
    """
    if num == 0:
        return UKY_NUMBER_MAP[0]
    components = decompose_number(num)
    result = []
    for unit, count in components:
        if unit == 1:  # 个位处理
            result.append(UKY_NUMBER_MAP[count])
        elif unit == 10:  # 十位处理（如20、30等）
            result.append(UKY_NUMBER_MAP[unit * count])
        else:  # 其他单位处理
            count_str = int_to_uky(count)
            result.append(f"{count_str} {UKY_NUMBER_MAP[unit]}")
    return " ".join(result)


def float_to_uky(num: float) -> str:
    """
    将浮点数转换为uky数字表示
    :param num: 浮点数
    :return: uky数字表示的浮点数字符串
    """
    integer_part = int_to_uky(int(num))
    decimal_part = int_to_uky(decimal_number(num))
    if decimal_part == '0':
        return integer_part
    decimal_places_count = decimal_places(num)
    decimal_places_unit = UKY_NUMBER_MAP.get(10 ** decimal_places_count, int_to_uky(10 ** decimal_places_count))

    return f"{integer_part} пүтүн {decimal_places_unit} дә {decimal_part}"
...
def number_hyphen_reverse(text: str) -> str:
    ...
    def replacement(match) -> str:
        ...
        if prefix in UKY_REVERSE_NUMBER_MAP.keys():
            return ' ' + prefix + ' - '
        elif prefix + 'и' in UKY_REVERSE_NUMBER_MAP.keys():
            return ' ' + prefix + 'и' + ' - '
        elif prefix + 'ә' in UKY_REVERSE_NUMBER_MAP.keys():
            return ' ' + prefix + 'ә' + ' - '
        else:
...
def uky_punctuation_to_other(s: str, target: int) -> str:
    """
    将UKY标点符号转换成UEY,ULY,UYY,IPA 的标点符号
    :param s: 要转换的字符串
    :param target: 目标标点符号类型，值应为 0,1,2,4 (表示 UEY,ULY,UYY,IPA) 中的一个
    :return: 处理结果
    """
    if target not in [0, 1, 2, 4]:
        raise ValueError("Invalid target punctuation type")
    for punctuation in PUNCTUATIONS:
        s = s.replace(punctuation[3], punctuation[target])
    return s
...
class UKYNumberProcessor(Processor):
    ...
    def IntegerNumber(self, text: str) -> str:
        ...
        def replace(match) -> str:
            ...
            # 转换数字
            number = int_to_uky(int(number_str))
    ...
    def FloatNumber(self, text: str) -> str:
        ...
        def replace(match) -> str:
            ...
            suffix = suffix_part.replace(after_assertion, '').strip()
            # 转换数字
            number = float_to_uky(float(number_str))
...
class UKYNumberReverseProcessor(Processor):
    """
    吧UKY 数字转换成阿拉伯数字,逻辑跟 UEY 样一。详细的内容参考 UEYNumberReverseProcessor
    """
    ...
    def reverse_float(self, text: str):
        ...
        for word in tokens:
            if word in UKY_REVERSE_NUMBER_MAP:
                if not prefix:
    ...
    def reverse_integer(self, text: str) -> str:
        ...
        for word in tokens:
            if word in UKY_REVERSE_NUMBER_MAP:
                pending.append((UKY_REVERSE_NUMBER_MAP[word], UKY_REVERSE_NUMBER_MAP[word] < 100))
                continue
            if pending:
...
class UKYDateProcessor(Processor):
    def __init__(self):
        super().__init__(ProcesClass.UKY, ProcesType.DATE, "DefaultUKYDateProcessor")
        self.date_regex_map = {
            'iso': {  # 格式1：YYYY-MM-DD
                'regex_year': r'\d{1,4}',
...
class UKYToIPAProcessor(Processor):
    """
    UKY 转换到 IPA
    """
...
class UKYToUYYProcessor(Processor):
    """
    UKY 转换到 UYY
    """
...
class UKYToUEYProcessor(Processor):
    """
    UKY 转换到 UEY
    """
    ...
    def process(self, text: str) -> str:
        ...
        """
        text = text.lower()
        text = (self.uky_divide_syllables_processor.process(text=text, delimiter='<dlmt>')
                .replace('ю', 'йу')
                .replace('я', 'йа')
                .split('<dlmt>'))
        result = []
...
class UKYToULYProcessor(Processor):
    """
    UKY 转换到 ULY
    """
...
class UKYDivideSyllablesProcessor(Processor):
    """
    UKY 音节划分
    ...
    def process(self, text: str, split_vowels: bool = True, delimiter: str = ''):
        ...
        :return: 转换后的音节字符串
        """
        text.replace('ю', 'йу').replace('я', 'йа')
        syllable_list = []
        ...
        return f' {delimiter} '.join(syllable_list).replace('йу', 'ю').replace('йа', 'я')
